...
export async function PATCH(request: Request, { params }: RouteContext) {
  ...
  if (!Number.isInteger(id) || !status) return NextResponse.json({ error: "Valid order id and status are required." }, { status: 400 });

  try {
    const order = await prisma.order.update({ where: { id }, data: { status } });
    return NextResponse.json(serializeOrder(order));
  } catch (error) {
    if (typeof error === "object" && error !== null && "code" in error && error.code === "P2025") return NextResponse.json({ error: "Order not found." }, { status: 404 });
    throw error;
  }
}
